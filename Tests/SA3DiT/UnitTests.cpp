...
auto tAddTensors = test("SA3DiT/addTensorsMatchesReference") = []
{
    auto& device = Device::shared();
    ...
    {
        auto pass = commands.beginCompute();
        result = addTensors(pass, a, b, device);
    }
...
auto tConcatAndSliceRows = test("SA3DiT/concatRowsThenSliceRowsRoundTrips") = []
{
    auto& device = Device::shared();
...
auto tSliceColumns = test("SA3DiT/sliceColumnsMatchesReference") = []
{
    auto& device = Device::shared();
