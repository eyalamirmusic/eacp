...
//   }
//   auto pass = frame.beginPass();          // samples rasterizer.coverage()
//
...
public:
    ...
    // Uploads the path's segments and sizes the coverage texture to its bounds.
    // Every sub-path is closed, since a fill always treats one as closed.
    ...
    // The rect in path units the coverage texture spans: the path's bounds
    // snapped out to whole pixels and grown by one, so an edge landing on a
    // pixel boundary still has somewhere to spill its coverage.
    Graphics::Rect getCoveredBounds() const { return covered; }

    // The coverage mask, one texel per device pixel of getCoveredBounds(), with
    // the same value in all four channels. Only valid while !isEmpty().
...
private:
    void resizeCoverage(int width, int height);
    void uploadSegments();
    ...
    // Directed segments in coverage-texture pixel space, four floats each. Kept
    // between uploads so a re-rasterization at the same size reuses the buffer.
