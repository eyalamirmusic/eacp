...
struct VulkanRenderPipelineBackend final : RenderPipelineBackend
{
    VulkanRenderPipelineBackend(Device& device,
    ...
    ~VulkanRenderPipelineBackend() override
    {
    ...
    {
        return const_cast<VulkanRenderPipeline*>(&state);
    }

    void* nativeDepthState() const override
    {
        const auto tests = state.depth || state.stencil;

        return tests ? const_cast<VulkanRenderPipeline*>(&state) : nullptr;
    }
};
} // namespace
...
std::unique_ptr<RenderPipelineBackend>
    makeVulkanRenderPipeline(Device& device,
                             const RenderPipelineDescriptor& descriptor)
{
    return std::make_unique<VulkanRenderPipelineBackend>(device, descriptor);
}
} // namespace eacp::GPU
