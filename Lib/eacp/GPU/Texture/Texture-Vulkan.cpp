...
namespace
{
struct VulkanTextureBackend final : TextureBackend
{
    VulkanTextureBackend(Device& device,
    ...
    VulkanTextureBackend(Device& device, void*)
        : context(getVulkanContext(device))
    {
    }

    ~VulkanTextureBackend() override { release(); }
    ...
    {
        return isValid() && data.isRenderTarget();
    }
    ...
    {
        return isValid() && data.isComputeWritable();
    }

    bool hasDepth() const override { return isValid() && data.hasDepth(); }

    bool hasStencil() const override { return isValid() && data.hasStencil(); }

    bool hasSampleableDepth() const override
    {
        return isValid() && data.hasSampleableDepth();
    }
...
{
    return std::make_unique<VulkanTextureBackend>(device, descriptor, pixels);
}

// No zero-copy pixel-buffer path on Linux: the texture comes out invalid, as it
// does on D3D12, and Texture::update is the path.
std::unique_ptr<TextureBackend> wrapVulkanPixelBuffer(Device& device,
                                                      void* nativePixelBuffer)
{
    return std::make_unique<VulkanTextureBackend>(device, nativePixelBuffer);
}
} // namespace eacp::GPU
