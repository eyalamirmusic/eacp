...
// Screen tier feeds the same input type through appendBGRA, straight from the
// compositor. The GpuDirect tier is not wired here yet (no D3D->MF zero-copy),
// so those hooks keep the base's "unsupported" default and callers fall back to
// Snapshot.
...
// drains audio on a thread this encoder never created -- as does the Screen
// tier, whose frames arrive on a thread pool. Balanced at thread exit.
...
bool WindowsEncoder::begin(const FilePath& path, const EncoderSpec& spec)
{
...
bool WindowsEncoder::configureStreams(const EncoderSpec& spec)
{
...
bool WindowsEncoder::configureAudioStream(const AudioSpec& spec)
{
...
void WindowsEncoder::writeVideoBuffer(IMFMediaBuffer* buffer, double ptsSeconds)
{
...
void WindowsEncoder::appendImage(const Graphics::Image& image, double ptsSeconds)
{
    if (!writer)
        return;
    ...
    writeVideoBuffer(buffer.Get(), ptsSeconds);
}
                                ...
                                double ptsSeconds)
{
    ...
    // seen; the audio drain is the other one.
    ...
    auto copyWidth = std::min(width, sourceWidth);
    auto copyHeight = std::min(height, sourceHeight);

    // A window that shrank mid-recording leaves the rest of the frame unwritten,
    // and the memory buffer arrives holding whatever was in it.
...
void WindowsEncoder::appendAudio(const AudioBuffer& buffer, double ptsSeconds)
{
    thread_local auto com = ComOnThisThread {};
    ...
    sample->SetSampleTime(toMediaTime(ptsSeconds));
    sample->SetSampleDuration(toMediaTime((double) frames / audioSpec->sampleRate));
